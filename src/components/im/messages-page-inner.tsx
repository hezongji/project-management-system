...
/**
 * MessagesPageInner —— IM 会话页共享组件（W1 抽取，2026-08-29）
 *
 * 由 /messages（desktop 双栏）与 /im（mobile 单栏，独立聊天 App 壳）共用。
 * mode='mobile'：列表↔聊天切换态（微信式），高度 h-dvh，不依赖 (main) 布局。
 *
...
import { useConversations, useChatSocket, useMessages, type ConversationItem } from '@/components/im/use-im-hooks'
...
import { MessageSquare, Send, Hash, Users, Paperclip, X, Plus, Loader2, Sparkles, Trash2, ArrowLeft } from 'lucide-react'
...
export function MessagesPageInner({ mode = 'desktop' }: { mode?: 'desktop' | 'mobile' }) {
  ...
  // ── 当前会话历史消息 + 已读回执（W1：抽 hooks）──
  const { messages, msgsLoading, readUserIds, setReadUserIds } = useMessages(selectedId, meId)
  ...
  // ── Socket.IO（W1：抽 hooks，事件回调经 latestRef 读取，依赖仅稳定引用）──
  const { socketRef, sendText, sendFileMessage, revoke } = useChatSocket({
    selectedIdRef,
    onMessagesUpdate: (cid) => queryClient.invalidateQueries({ queryKey: ['conversation-messages', cid] }),
    onConversationsUpdate: () => queryClient.invalidateQueries({ queryKey: ['conversations'] }),
    setReadUserIds,
  })
  ...
  const confirmUpload = async () => {
    ...
    if (!file || !selectedId || !uploadCatalogId) return
    setUploading(true)
    try {
      ...
      sendFileMessage({
        conversationId: selectedId,
        file: { name: f.name, size: f.size, mimeType: f.mimeType, fileId: f.id },
        isImage,
      })
      setPendingFile(null)
    } catch {
  ...
  return (
    <div className={cn(
      'flex gap-4',
      isMobile ? 'h-dvh flex-col gap-0' : 'h-[calc(100vh-8rem)]',
    )}>
      {/* ── 会话列表（mobile 选中会话后隐藏）── */}
      <Card className={cn(
        'flex flex-col overflow-hidden',
        isMobile ? 'h-full w-full' : 'w-72 shrink-0',
          ...
          <>
            <div className="flex items-center gap-2 border-b px-4 py-3">
              ...
              <div className="ml-auto flex shrink-0 items-center gap-2">
                {/* 解散会话（删除工程第5棒）：仅群主可见；mobile 隐藏（窄屏溢出） */}
                {!isMobile && current?.myRole === 'OWNER' && (
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                ...
                {/* 生成会议纪要（S4，AI）——mobile 隐藏（窄屏溢出，二期收进菜单） */}
                {!isMobile && (
                <Button
                  type="button"
                  variant="outline"
