...
const S5_PASSWORD = process.env.S5_PASSWORD || 'demo123456'
...
try {
  ...
  const login = await api('POST', '/auth/login', { body: { email: 'chenmuzhi@example.com', password: S5_PASSWORD } })
  ...
  assert(!!cf1.body?.data?.confirmedAt, 'confirmedAt 已落库（收货留痕 ✓）')
  assert(cf1.body?.data?.confirmedAt && cf1.body?.data?.orderCompleted === false, 'confirm 响应含留痕+状态')
  ...
  const u = await api('GET', `/users?page=1&limit=20`, { token }).then(r => (r.body?.data?.items ?? []).find(x => /@example\.cn$/.test(x.email ?? '')))
  ...
  // 清理：删全部 S5SMOKE 数据（S5_DATABASE_URL 可指向生产库直接清理；否则走 API cancel）
  header('清理 S5SMOKE 测试数据')
  try {
    if (process.env.S5_DATABASE_URL) {
      const pr2 = new PrismaClient({ datasources: { db: { url: process.env.S5_DATABASE_URL } } })
      await pr2.$transaction([
        pr2.goodsArrivalItem.deleteMany({ where: { arrival: { order: { OR: [{ title: { contains: 'S5SMOKE' } }, { code: { contains: 'S5SMOKE' } }] } } } }),
        pr2.goodsArrival.deleteMany({ where: { order: { OR: [{ title: { contains: 'S5SMOKE' } }, { code: { contains: 'S5SMOKE' } }] } } }),
        pr2.purchaseOrderItem.deleteMany({ where: { order: { OR: [{ title: { contains: 'S5SMOKE' } }, { code: { contains: 'S5SMOKE' } }] } } }),
        pr2.purchaseContract.deleteMany({ where: { order: { OR: [{ title: { contains: 'S5SMOKE' } }, { code: { contains: 'S5SMOKE' } }] } } }),
        pr2.purchaseOrder.deleteMany({ where: { OR: [{ title: { contains: 'S5SMOKE' } }, { code: { contains: 'S5SMOKE' } }] } }),
      ])
      await pr2.$disconnect()
      log('  已用 S5_DATABASE_URL 直连清理')
    } else {
      // 无直连时通过 API 取消并标记（尽力而为，脚本内不再依赖本机库）
      const list = await api('GET', '/purchase-orders?page=1&limit=100', { token })
      const smokes = (list.body?.data?.items ?? []).filter((o) => /S5SMOKE/.test(o.code ?? '') || /S5SMOKE/.test(o.title ?? ''))
      for (const o of smokes) {
        try { await api('PATCH', `/purchase-orders/${o.id}`, { token, body: { action: 'cancel' } }) } catch { /* ignore */ }
      }
      log(`  已通过 API 取消 ${smokes.length} 单 S5SMOKE（如需彻底删除请配 S5_DATABASE_URL）`)
    }
  } catch (e) {
